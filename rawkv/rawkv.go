...
func (c *Client) CompareAndSwap(ctx context.Context, key, previousValue, newValue []byte, options ...RawOption) ([]byte, bool, error) {
	return c.CompareAndSwapWithTTL(ctx, key, previousValue, newValue, 0, options...)
}

// CompareAndSwapWithTTL is the same as CompareAndSwap, but with a time-to-live duration.
func (c *Client) CompareAndSwapWithTTL(ctx context.Context, key, previousValue, newValue []byte, ttl uint64, options ...RawOption) ([]byte, bool, error) {
	if !c.atomic {
